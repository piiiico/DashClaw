...
The canonical Node SDK currently exposes **87 public methods** in `sdk/dashclaw.js` and the Python SDK **227** in `sdk-python/dashclaw/client.py` (both reproducible via `npm run sdk:count` — excludes the constructor and `_`-private methods). The Node surface includes:
