...
| Fact | Source |
|:---|:---|
| Complete API route inventory | `docs/api-inventory.md` and `docs/api-inventory.json` generated from `app/api/**/route.js` |
...
The canonical Node SDK currently exposes **104 public methods** in `sdk/dashclaw.js` and the Python SDK **203** in `sdk-python/dashclaw/client.py` (both reproducible via `npm run sdk:count` — excludes the constructor and `_`-private methods). The Node surface includes:
...
| Library | Role |
|:---|:---|
| `app/lib/db.js` | Neon/Postgres connection. |
| `app/lib/org.js` | Org resolution, scoping, and roles. |
| `app/lib/guard.js` | Policy evaluation engine. |
| `app/lib/repositories/actions.repository.js` | Action ledger, durable outcome, idempotency, trace/graph data access. |
| `app/lib/security.js` | Sensitive-data scanning and redaction. |
| `app/lib/events.js` | Org-scoped event publish layer. |
| `app/lib/signals.js` | Runtime signal/anomaly computation. |
| `app/lib/readiness.mjs` | Setup/readiness checks. |
| `app/lib/doctor/` | Shared diagnostic engine for API, CLI, and local doctor scripts. |
| `app/lib/integration-health.js` | Integration credential health checks. |
| `app/lib/notification-adapters/` | Slack, Discord, Linear, GitHub, Email notification adapters. |
| `app/lib/telegramApprovals.js` | Telegram approval messages and inline callbacks. |
| `app/lib/discordApprovals.js` | Discord approval messages and interaction bridge. |
| `app/lib/capability-invoke.js` | Governed HTTP capability invocation. |
| `app/lib/workflow-executor.js` | Sequential governed workflow execution. |
| `app/lib/step-handlers.js` | Workflow step handlers. |
| `app/lib/template-vars.js` | Template variable substitution. |
| `app/lib/usage.js` | Plan limits, quota enforcement, metering, and cost estimation. |
| `app/lib/billing.js` | Token-cost pricing helpers. |
| `app/lib/policy-generator.js` | Natural-language policy generation. |
| `app/lib/predictive-risk.js` | Statistical and optional LLM-assisted risk scoring. |
