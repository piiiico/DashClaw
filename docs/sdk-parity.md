...
|---|---|---|---|---|
| Guard / actions / approvals | Yes | Yes | Yes | Stable, canonical in main SDK. Approvals use `POST /api/approvals/[actionId]` (shared by browser, CLI, `/approve` mobile PWA, SDK polling). Phase 1 agent attribution (`agent_id`, `agent_name`) supported in both SDKs — pass in constructor or per-call body. Phase 2 JWKS verification shipped (#104): pass `authToken` in the Node SDK constructor for cryptographic attribution; server returns `verification_status` on every guard response. Python SDK Phase 2 parity is pending. See `docs/agent-identity.md`. |
