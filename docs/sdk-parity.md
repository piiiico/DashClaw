...
| Action outcome (durable execution finality) | Phase 1: HTTP only, Node wrapper pending | n/a | Phase 1: HTTP only, Python wrapper pending | `POST/GET /api/actions/:id/outcome` shipped in Phase 1. SDK wrappers (`reportActionOutcome` / `getActionOutcome`) follow in Phases 3 + 4. See `docs/architecture/durable-execution-finality.md`. |
