 ...
 */
export default function ProofExportPanel({ open, onClose }: ProofExportPanelProps) {
  ...
  const load = useCallback(async (fmt: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const formatButton = (value: string, label: string) => (
    <button
      type="button"
