  ...
  return (
    <div className="p-8">
      <header className="mb-6">
        ...
        <p className="text-sm text-zinc-500">Claude Code session analytics (hooks + JSONL backfill).</p>
        {unread > 0 && (
      ...
      {!projects.length ? (
        <div className="rounded-md border border-dashed border-zinc-300 p-8 text-center text-zinc-600">
          ...
          <p className="mt-2 text-sm">
            Enable the Stop hook reporter (<code>DASHCLAW_CODE_SESSIONS_ENABLED=1</code>),
            or backfill via <code>dashclaw code ingest</code>.
          </p>
        </div>
      ) : (
            ...
            {projects.map(p => (
              <tr key={p.id} className="border-b last:border-0 hover:bg-zinc-50">
                <td className="py-3 pr-3">
                  <Link className="font-medium text-orange-700 underline-offset-2 hover:underline"
                        href={`/code-sessions/${p.id}`}>
                    {p.slug}
                  </Link>
                  {p.cwd && <div className="mt-1 text-xs text-zinc-500">{p.cwd}</div>}
                </td>
