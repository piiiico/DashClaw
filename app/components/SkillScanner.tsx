...
// `webkitdirectory`/`directory` are non-standard folder-picker attributes not in
// React's built-in input typings; declare them so the existing markup typechecks
// unchanged (DOM output is identical).
...
const SEVERITY_VARIANT: Record<string, string> = { high: 'error', medium: 'warning', low: 'info' };
...
export default function SkillScanner() {
  ...
  const [files, setFiles] = useState<SkillFile[]>([{ filename: '', content: '' }]);
  ...
  const updateFile = (i: number, key: keyof SkillFile, value: string) => {
    setFiles((prev) => prev.map((f, idx) => (idx === i ? { ...f, [key]: value } : f)));
  };
  const addFile = () => setFiles((prev) => [...prev, { filename: '', content: '' }]);
  const removeFile = (i: number) => setFiles((prev) => (prev.length === 1 ? prev : prev.filter((_, idx) => idx !== i)));
    ...
    for (const file of picked) {
      ...
      try {
        if (/\.zip$/i.test(file.name)) {
          ...
          add(rel, text);
          if (!inferredName && (file as any).webkitRelativePath) inferredName = (file as any).webkitRelativePath.split('/')[0];
        }
      } catch {
        skipped += 1;
      }
  ...
  const onPick = (e: React.ChangeEvent<HTMLInputElement>) => { ingestFiles(e.target.files); e.target.value = ''; };
