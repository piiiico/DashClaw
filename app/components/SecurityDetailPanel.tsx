...
interface SignalDetailProps {
  signal: any;
  onClose: () => void;
  onDismiss?: ((signal: any) => void) | null;
}

function SignalDetail({ signal, onClose, onDismiss }: SignalDetailProps) {
...
function ActionDetail({ action }: { action: any }) {
      ...
      {parseSideEffects(action.side_effects).length > 0 && (
        <div>
          ...
          <ul className="text-sm text-secondary list-disc pl-5 space-y-0.5">
            {parseSideEffects(action.side_effects).map((se: any, i: number) => (
...
export default function SecurityDetailPanel({ item, type, onClose, onDismiss }: SecurityDetailPanelProps) {
