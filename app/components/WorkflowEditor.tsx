 ...
 */
function stepsToFlow(steps: any): { nodes: any[]; edges: any[] } {
  if (!steps) {
    return {
      nodes: [
        {
 ...
 */
function flowToSteps(nodes: any[], edges: any[]) {
  return {
    nodes: nodes.map((n) => ({
      id: n.id,
...
export default function WorkflowEditor({ steps, onChange, readOnly = false }: WorkflowEditorProps) {
