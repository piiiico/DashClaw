...
// Parsed-JSON values from the rollout file are inherently loosely shaped.
...
function _initState({ sourceFile, sourceMtime }: { sourceFile?: string | null; sourceMtime?: string | null }): ParserState {
  return {
    session: {
...
function _handleSessionMeta(state: ParserState, payload: JsonRecord | undefined, ts: string | null): void {
  const s = state.session;
  if (!s.sessionUuid && payload?.id) s.sessionUuid = payload.id as string;
  if (!s.cwd && payload?.cwd) s.cwd = payload.cwd as string;
  if (!s.cliVersion && payload?.cli_version) s.cliVersion = payload.cli_version as string;
  if (!s.originator && payload?.originator) s.originator = payload.originator as string;
  if (!s.modelProvider && payload?.model_provider) s.modelProvider = payload.model_provider as string;
  if (!s.startedAt) s.startedAt = ts || (payload?.timestamp as string | undefined) || null;
  if (payload?.git && typeof payload.git === 'object') {
    const git = payload.git as JsonRecord;
    if (!s.gitBranch && git.branch) s.gitBranch = git.branch as string;
    if (!s.gitCommit && git.commit_hash) s.gitCommit = git.commit_hash as string;
    if (!s.gitRepoUrl && git.repository_url) s.gitRepoUrl = git.repository_url as string;
  }
}

function _handleResponseItem(state: ParserState, payload: JsonRecord | undefined, ts: string | null): void {
  const s = state.session;
  s.responseItems += 1;
  ...
  if (payload.type === 'function_call' || (payload.name && payload.call_id)) {
    s.toolCallCount += 1;
    s.toolUses.push({
      name: (payload.name as string | undefined) || 'unknown',
      tool_use_id: (payload.call_id as string | null | undefined) || (payload.id as string | null | undefined) || null,
    ...
    s.messages.push({
      ...
      tool_use_id: (payload.call_id as string | null | undefined) || null,
...
function extractTextPreview(content: unknown[]): string {
  for (const c of content) {
    ...
    const part = c as JsonRecord;
    if (part.type === 'input_text' && typeof part.text === 'string') return previewText(part.text);
    if (part.type === 'output_text' && typeof part.text === 'string') return previewText(part.text);
    if (part.type === 'text' && typeof part.text === 'string') return previewText(part.text);
  }
  return '';
}

function safeToolTarget(p: JsonRecord): string | null {
...
function _handleEventMsg(state: ParserState, payload: JsonRecord | undefined, ts: string | null): void {
  const s = state.session;
  s.eventMessages += 1;
  ...
  if (type === 'token_count') {
    ...
    const last = (payload.info as JsonRecord | undefined)?.last_token_usage as JsonRecord | undefined;
    if (last) {
  ...
  if (type === 'session_configured') {
    if (!s.modelPrimary && payload.model) s.modelPrimary = payload.model as string;
    return;
  }
...
function _handleCompacted(state: ParserState, payload: JsonRecord | undefined, ts: string | null): void {
  const s = state.session;
  s.compactedItems += 1;
  s.messages.push({
    role: 'system',
    preview: '(compacted) ' + previewText((payload?.message as string | undefined) || '', 180),
    at: ts,
  });
}

function _processLine(state: ParserState, line: string, lineNo: number): void {
  if (!line || !line.trim()) return;
  const rec = safeParse(line) as JsonRecord | null;
  const s = state.session;
...
export function parseCodexSessionLines(
  lines: string[],
  { sourceFile = null, sourceMtime = null }: { sourceFile?: string | null; sourceMtime?: string | null } = {},
): CodexSession {
