 ...
 *
 * @param {Function} sql
 * @returns {Promise<{kid:string, privateKeyJwk:object, publicKeyJwk:object, source:'env'|'db'}>}
 */
export async function getServerSigningKey(sql) {
 ...
 */
export async function getServerPublicJwks(sql) {
  const keys = [];
  try {
    const env = parseEnvSigningKey();
  ...
  try {
    const dbKeys = await listPublicJwks(sql);
...
export function _resetSigningKeyCacheForTesting() {
  cached = null;
}
