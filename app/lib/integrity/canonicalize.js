 ...
 */
function assertBoundedDepth(value, depth) {
  if (depth > MAX_JSON_DEPTH) {
    const err = new Error('integrity: JSON nesting exceeds max depth');
    err.code = 'MAX_DEPTH';
    throw err;
  }
  if (Array.isArray(value)) {
    ...
    for (const k in value) assertBoundedDepth(value[k], depth + 1);
  }
}
...
function nfcDeep(value) {
  ...
  if (value && typeof value === 'object') {
    const out = {};
    for (const [k, v] of Object.entries(value)) out[k.normalize('NFC')] = nfcDeep(v);
    return out;
  }
  return value;
}
