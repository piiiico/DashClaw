 ...
 *
 * @typedef {{ code: string, label: string, detail?: string }} Violation
 * @typedef {{ verdict: 'pass'|'block', violations: Violation[] }} VerifyResult
 */
...
function wordBoundary(term, flags) {
 ...
 */
function assertSafePattern(src) {
  if (typeof src !== 'string' || src.length > MAX_PATTERN_LENGTH) {
    const err = new Error('non_fabrication: unsafe or oversized pattern');
    err.code = 'UNSAFE_PATTERN';
    throw err;
  }
  const stack = []; // per group-nesting level: does the body contain a quantifier?
  let escaped = false;
  let inClass = false;
  for (let i = 0; i < src.length; i++) {
    ...
    if (c === ')') {
      ...
      if (quantified && grp.hasQuant) {
        const err = new Error('non_fabrication: unsafe nested-quantifier pattern');
        err.code = 'UNSAFE_PATTERN';
        throw err;
      }
      if (stack.length && (grp.hasQuant || quantified)) stack[stack.length - 1].hasQuant = true;
      continue;
    }
    if (c === '+' || c === '*') {
    ...
    if (c === '{') {
      ...
      if (/^\{\d+,\}/.test(src.slice(i)) && stack.length) stack[stack.length - 1].hasQuant = true;
    }
  }
}
 ...
 */
function isRequiredFactSatisfied(text, fact) {
 ...
 * @param {object} source - { requiredFacts, allowedFacts, forbiddenPatterns?, extract? }
 * @returns {VerifyResult}
 */
export function verify(candidate, source) {
  try {
    const violations = [];
