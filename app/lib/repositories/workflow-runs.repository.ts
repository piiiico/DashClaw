...
export function shapeRun(row: Row | null | undefined): ShapedRun | null {
  if (!row) return null;
  ...
  return {
    ...
    step_count: parseInt(row.step_count as string, 10) || 0,
    steps_completed: parseInt(row.steps_completed as string, 10) || 0,
    steps_failed: parseInt(row.steps_failed as string, 10) || 0,
  };
}

interface ShapedStepResult {
  step_result_id: unknown;
  step_id: unknown;
  step_index: unknown;
  step_type: unknown;
  step_name: unknown;
...
export function shapeStepResult(row: Row | null | undefined): ShapedStepResult | null {
  if (!row) return null;
  return {
...
interface InsertStepResultPayload {
  stepResultId: string;
  runActionId: string;
  orgId: string;
  templateId: string;
...
export async function insertStepResult(
  sql: SqlTag,
  { stepResultId, runActionId, orgId, templateId, stepData }: InsertStepResultPayload,
): Promise<void> {
...
interface UpdateStepResultPayload {
  runActionId: string;
  orgId: string;
  stepData: StepData;
}
...
export async function updateStepResult(
  sql: SqlTag,
  { runActionId, orgId, stepData }: UpdateStepResultPayload,
): Promise<void> {
...
interface ResumeContext {
  ...
  priorSteps: Record<string, { output: unknown }>;
...
export function buildResumeContext(
  ...
  const priorSteps: Record<string, { output: unknown }> = {};
  for (const step of stepResults) {
    ...
    priorSteps[step.step_id as string] = {
      output: step.output,
    };
  }
...
interface ListWorkflowRunsFilters {
  status?: string;
  agent_id?: string;
  limit?: number | string;
...
): Promise<{ runs: (ShapedRun | null)[]; total: number }> {
...
interface CancelWorkflowRunResult {
  found: boolean;
  running?: boolean;
  status?: unknown;
}
...
export async function getWorkflowRun(
  ...
  const reasoning = safeJsonParse(actionRows[0]?.reasoning) as { template_name?: unknown } | null;
  ...
  const steps = stepRows.map(shapeStepResult);
  run.steps = steps;
  run.step_count = steps.length;
  run.steps_completed = steps.filter((s) => s?.status === 'completed').length;
  run.steps_failed = steps.filter((s) => s?.status === 'failed').length;
