...
type CheckAndRecordResult = 'unique' | 'replayed' | 'unavailable';
...
export async function checkAndRecord(
  sql: SqlTag,
  { jti, issuer, expiresAt, agentId }: { jti: string; issuer: string; expiresAt: number; agentId?: string | null },
): Promise<CheckAndRecordResult> {
  if (!jti || !issuer || typeof expiresAt !== 'number') {
    const err = Object.assign(new Error('checkAndRecord: jti, issuer, expiresAt are required'), { code: 'INVALID_INPUT' });
    throw err;
  }
  ...
  if (jti.length > MAX_JTI_LENGTH) {
    const err = Object.assign(new Error(`checkAndRecord: jti exceeds ${MAX_JTI_LENGTH} chars (got ${jti.length})`), { code: 'OVERSIZED_JTI' });
    throw err;
  }
  ...
  try {
    ...
    if (Math.random() < SWEEP_PROBABILITY) {
      void sql`DELETE FROM jwt_replay_log WHERE expires_at < ${seenAt}`.catch((err) => {
    ...
    console.warn('[jti-replay] store unavailable:', (err as Error)?.message || err);
    return 'unavailable';
  }
}
 ...
 */
export async function sweep(sql: SqlTag): Promise<number> {
  await ensureTable(sql);
