 ...
 */
export async function getCachedScan(sql: SqlTag, orgId: string, skillName: string, targetHash: string): Promise<Record<string, unknown> | null> {
 ...
 * updated row.
 */
export async function upsertScan(sql: SqlTag, orgId: string, input: UpsertScanInput): Promise<Record<string, unknown> | undefined> {
 ...
 */
export async function getScanById(sql: SqlTag, orgId: string, id: string): Promise<Record<string, unknown> | null> {
  const rows = await sql`
    SELECT id, org_id, skill_name, target_hash, findings, passed, created_at
    FROM skill_scan_results
