...
export function isEnforceable(kind) {
  return ENFORCEABLE_KINDS.includes(kind);
}

/** Parse a sample timestamp to epoch ms; NaN-safe (returns 0 on garbage). */
export function tsMs(sample) {
  const t = Date.parse(sample && sample.ts);
  return Number.isFinite(t) ? t : 0;
}

function isCommandSample(sample) {
...
export function decideSample(rule, sample) {
  ...
  switch (rule.kind) {
    ...
    case RULE_KINDS.MODEL_TASK_MISMATCH_WARN: {
      ...
      if (!isBelowTier(sample.model, rule.min_tier || DEFAULTS.modelMinTier)) return 'allow';
...
export function sampleMatchesAllowlist(rule, sample) {
 ...
 */
export function detectReloadLoops(samples, { maxReloads, windowMinutes } = {}) {
 ...
 */
export function detectFailureLoops(samples, { maxFailures, windowMinutes } = {}) {
  ...
  const flagged = new Set();
  const ordered = [...samples].sort((a, b) => tsMs(a) - tsMs(b));
  const fails = new Map(); // key -> [{ts, event_id}]

  for (const s of ordered) {
    if (s.outcome_status !== 'failed') continue;
    const key = `${s.tool}|${s.command_shape || s.action_type || ''}`;
    const t = tsMs(s);
    let list = (fails.get(key) || []).filter((r) => t - r.ts <= windowMs);
    list.push({ ts: t, event_id: s.event_id });
    fails.set(key, list);
    if (list.length >= max) {
      for (const r of list) flagged.add(r.event_id);
 ...
 */
export function evaluateRuleOverSamples(rule, samples) {
 ...
 */
export function behaviorRuleToGuardPolicy(rule, { agentId, name } = {}) {
