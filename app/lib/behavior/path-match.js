/**
 * Protected-path matching shared by the guard engine (`protected_path` policy
 * type) and the Behavior Learning analyzer/simulator. Defining the matcher and
 * the default protected groups in ONE place guarantees that what the Policy
 * Coach simulates against local samples is exactly what the guard would enforce
 * once a draft is activated.
 *
...
/**
 * Canonical protected groups for V1. Each group maps a label to a set of globs.
 * These mirror the high-blast-radius surfaces named in the Behavior Learning
 * spec: auth, middleware, billing, secrets, .organism, livingcode, and
 * cron/gateway config. Globs are intentionally broad (`**`) so a touch anywhere
 * in the tree is caught regardless of where the agent's cwd sits.
 */
export const PROTECTED_PATH_GROUPS = {
...
export const DEFAULT_PROTECTED_PATHS = Object.values(PROTECTED_PATH_GROUPS).flat();
 ...
 */
export function matchesProtectedPath(path, patterns) {
...
/** Return the protected-group label for a path, or null if unprotected. */
export function classifyProtectedPath(path) {
  for (const [group, patterns] of Object.entries(PROTECTED_PATH_GROUPS)) {
    if (matchesProtectedPath(path, patterns)) return group;
  }
  return null;
}
