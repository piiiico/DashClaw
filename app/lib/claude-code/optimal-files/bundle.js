...
export function buildOptimalFilesBundle({
  ...
  const files = [];
  files.push(generateRootClaudeMd(analysis));
  files.push(generateSessionPack(analysis));
  for (const r of generatePathRules(analysis)) files.push(r);
  for (const h of generateHooksBundle(analysis)) files.push(h);
  files.push(generateRecipe(analysis));
  for (const s of generateSkillCandidates(analysis)) files.push(s);
...
function groupBundle(files) {
  const groups = { recommended_now: [], optional: [], not_recommended_yet: [] };
  for (const f of files) {
    const g = groups[f.group] || groups.optional;
...
export function sideBySidePath(abs) {
...
export function previewBundleMerge({ bundle, projectCwd, filePath, existingContent = null }) {
  ...
  const fullSelection = {
    ...
    acceptedBullets: [].concat(...plan.sharedSections.map(s =>
