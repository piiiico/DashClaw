...
interface SharedSection {
  ...
  candidateBullets: CandidateBullet[];
}
...
export function parseMarkdownSections(text: string | null | undefined): MarkdownSection[] {
  const lines = String(text || '').split(/\r?\n/);
  const sections: MarkdownSection[] = [];
  // Capture preamble as a synthetic section with level=0.
  let cur: MarkdownSection = { level: 0, heading: '__preamble__', headingRaw: '', body: [] };
...
export function normalizeHeading(h: string | null | undefined): string {
  return String(h || '')
    .toLowerCase()
    .replace(/[`*_]/g, '')
    .replace(/[^\w\s]+/g, ' ')
...
export function parseBullets(bodyLines: string[]): { bullets: Bullet[]; other: string[] } {
...
function normalizeBullet(line: string | null | undefined): string {
  return String(line || '')
    .replace(/^\s*[-*]\s+/, '')
    .replace(/`[^`]*`/g, ' ')
    .replace(/[^a-z0-9\s]+/gi, ' ')
...
export function bulletAlreadyPresent(generatedNorm: string, existingBullets: Bullet[]): boolean {
...
export function previewMerge(existing: string | null | undefined, generated: string | null | undefined): MergePreview {
  const ex = parseMarkdownSections(existing);
  const gn = parseMarkdownSections(generated);
  const exByHeading = new Map<string, MarkdownSection>();
  for (const s of ex) {
    if (s.heading === '__preamble__') continue;
    if (!exByHeading.has(s.heading)) exByHeading.set(s.heading, s);
  }

  const appendSections: AppendSection[] = [];
...
function isFooterSection(section: MarkdownSection | null | undefined): boolean {
...
export function applyMerge(existing: string | null | undefined, generated: string | null | undefined, selection?: MergeSelection | null): MergeResult {
  ...
  const acceptedBulletsByHeading = new Map<string, string[]>();
