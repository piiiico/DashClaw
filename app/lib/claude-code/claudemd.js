...
export function rankReadFiles(toolEvents) {
...
export function resolveInsideProject(target, projectCwd) {
...
export function summarizeFile(absPath, content) {
  const ext = path.extname(absPath || '').toLowerCase();
  const out = { kind: 'unknown', highlights: [] };
  if (typeof content !== 'string' || !content) return out;
  const lines = content.split(/\r?\n/);
  const push = (s) => { if (s && out.highlights.length < 20) out.highlights.push(s); };
  ...
  if (ext === '.md') {
    ...
    for (const line of lines.slice(0, 60)) {
      ...
      if (m) push(`heading: ${m[1].slice(0, 80)}`);
    }
    return out;
  }
 ...
 * @param {Object} input.project       Project row.
 * @param {Array}  input.toolEvents    Tool events (for read-frequency analysis).
 * @param {string} input.projectCwd    Project root (for absolute-path resolution).
 * @param {Map<string,string>} [input.projectFiles]
 *                                     Map of relative-path → file content. When
 *                                     omitted or a path is missing, the
 *                                     summary entry is a stub.
 * @param {Date}   [input.now]
 */
export function generateClaudeMd({ session, project, toolEvents, projectCwd, projectFiles = null, now }) {
