...
export function labelFor(kind: string): SignalLabel {
  return SIGNAL_LABELS[kind] || { label: kind, tone: 'flow', suggestion: '' };
}

const CONFIDENCE_RANK: Record<string, number> = { high: 3, medium: 2, low: 1 };

export function severityRank(sig: { confidence?: string; savings_usd?: number | string | null }): number {
  const conf = CONFIDENCE_RANK[sig.confidence as string] || 0;
