...
interface ResourceOptions {
  modelStrategies?: ResourceOption[];
  policies?: ResourceOption[];
  knowledgeCollections?: ResourceOption[];
  capabilities?: ResourceOption[];
  promptTemplates?: ResourceOption[];
  [key: string]: any;
}

interface WorkflowSaveAction {
  label: string;
  disabled?: boolean;
  onClick: () => void;
}

function toggleArrayValue(values: string[], candidate: string): string[] {
...
interface WorkflowLinkedResourcesSectionProps {
  draft: any;
  resourceOptions: ResourceOptions;
  onChange: (patch: Record<string, any>) => void;
  saveAction?: WorkflowSaveAction | null;
}
...
export default function WorkflowLinkedResourcesSection({
  ...
  onChange,
  saveAction = null,
}: WorkflowLinkedResourcesSectionProps) {
    ...
    <div className="rounded-xl border border-border bg-white/[0.02] p-5 space-y-5">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          ) : draft.linked_capability_tags.map((tag: string) => (
            <button
              key={tag}
              type="button"
              onClick={() => updateField('linked_capability_tags', draft.linked_capability_tags.filter((candidate: string) => candidate !== tag))}
