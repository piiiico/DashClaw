...
function truncateText(text: any, maxLen: number) {
  if (!text) return '';
...
function buildInterventionList(pendingActions: any[], openLoops: any[]) {
  const items: any[] = [];
...
export default function MissionControlPage() {
  ...
  const [decisionMetrics, setDecisionMetrics] = useState<any>(null);
  const [capabilityHealth, setCapabilityHealth] = useState<any[]>([]);
  const [capabilityHealthError, setCapabilityHealthError] = useState<string | null>(null);
          ...
          <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:items-center sm:gap-x-6 sm:gap-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-0.5 ${posture.bg} ${posture.border}`}>
                <div className={`h-1.5 w-1.5 rounded-full ${posture.color.replace('text-', 'bg-')} ${(posture as any).pulse ? 'animate-pulse' : ''}`} />
