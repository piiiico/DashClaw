...
function formatRelativeTime(ts: string | number | null | undefined) {
  if (!ts) return '—';
...
interface OperationsFeedItemProps {
  item: any;
...
export default function OperationsFeedItem({ item, onApprove, onDeny, onRetry, onDisable, onCancel }: OperationsFeedItemProps) {
  const dot = SEVERITY_DOT[item.severity] || SEVERITY_DOT.low;
  const pill = CATEGORY_PILL[item.category] || CATEGORY_PILL.signal || { label: 'Signal', color: '' };
