...
const permissionVariant: Record<string, string> = {
  danger: 'error',
...
interface TrustPosture {
  permission_level: string;
  identity_verified: boolean;
  signature_enforced: boolean;
  active_policies_count: number;
  approval_record: { total: number; allowed: number };
  blocks_30d: number;
}

interface AgentTrustPostureProps {
  trust: TrustPosture;
}

export default function AgentTrustPosture({ trust }: AgentTrustPostureProps) {
