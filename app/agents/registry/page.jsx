...
export default function AgentRegistryPage() {
  ...
  const applyUpdatedAgent = useCallback((updated) => {
    if (!updated) return;
    setAgents((prev) => prev.map((a) => (a.entry_id === updated.entry_id ? { ...a, ...updated } : a)));
    setDetail((prev) => (prev ? { ...prev, registered_agent: { ...prev.registered_agent, ...updated } } : prev));
  }, []);
  ...
  const handleCapabilitiesChange = useCallback((next) => {
    setDetail((prev) => (prev ? { ...prev, capabilities: next } : prev));
  }, []);
