...
function UsageMeter({ label, icon: Icon, usage, limit, className = '' }: UsageMeterProps) {
...
function UsageContent() {
  ...
  const [costs, setCosts] = useState<any>(null); // /api/usage/costs — per-type + daily spend
            ...
            {Object.keys(costs.breakdown || {}).length > 0 && (
              <div className="space-y-1.5 mb-4">
                {Object.entries(costs.breakdown)
                  .sort((a: any, b: any) => b[1].cost_usd - a[1].cost_usd)
                  .map(([type, b]: [string, any]) => (
