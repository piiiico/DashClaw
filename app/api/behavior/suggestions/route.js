...
export async function GET(request) {
  try {
    ...
    console.error('[behavior/suggestions] GET error:', err.message);
...
export async function POST(request) {
  try {
    ...
    if (action === 'adopt') {
      ...
      const scoped = rule.agent_id ? samples.filter((s) => s.agent_id === rule.agent_id) : samples;
    ...
    console.error('[behavior/suggestions] POST error:', err.message);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
