...
function readConfig(map) {
  ...
  const until = map.BEHAVIOR_RECORDER_UNTIL || null;
  let expired = false;
  if (until) {
...
export async function GET(request) {
  try {
    ...
    const map = toMap(await getSettings(sql, orgId, { category: 'general' }));
    return NextResponse.json(readConfig(map));
  } catch (err) {
    console.error('[behavior/recorder] GET error:', err.message);
...
export async function POST(request) {
  try {
    ...
    console.error('[behavior/recorder] POST error:', err.message);
    const status = /Invalid setting key|Invalid category/.test(err.message) ? 400 : 500;
    return NextResponse.json({ error: status === 400 ? err.message : 'Internal server error' }, { status });
  }
}
