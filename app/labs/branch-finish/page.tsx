...
type SearchResults = SearchResultItem[] | { error: string } | null;
...
export default function BranchFinishPage() {
  ...
  // ---- Standards knowledge --------------------------------------------
  const [standardsCollection, setStandardsCollection] = useState<StandardsCollection | null>(null);
  ...
  const [decisions, setDecisions] = useState<LearningDecision[]>([]);
  ...
  const fetchStandards = useCallback(async () => {
    ...
    try {
      ...
      const d = await res.json();
      const match = (d.collections || []).find((c: StandardsCollection) =>
  ...
  const fetchCapabilities = useCallback(async () => {
    ...
    try {
      ...
      setCapabilitiesError((err as Error).message);
    } finally {
  ...
  const handleSelectTemplate = async (t: Template) => {
    ...
    try {
      ...
      (d.parameters || []).forEach((p: string) => { seeded[p] = ''; });
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      setSearchResults({ error: (err as Error).message });
    } finally {
      setSearching(false);
    }
  };
  ...
  const runScorer = async () => {
    ...
    try {
      ...
      setScorerError((err as Error).message);
    } finally {
      setScoring(false);
    }
  };
                    ...
                    <div className="space-y-2">
                      {renderResult.parameters.map((p) => (
                        <div key={p} className="flex items-center gap-2">
                          ...
                          <input
                            value={renderVars[p] ?? ''}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRenderVars((s) => ({ ...s, [p]: e.target.value }))}
                            placeholder="value"
            ...
            <CardContent>
              {standardsLoading ? (
                ...
                <div className="space-y-3">
                  ...
                  {searchResults && !Array.isArray(searchResults) && searchResults.error && (
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setScorerKeywords(e.target.value)}
                  ...
                  <textarea
                    value={scorerSample}
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setScorerSample(e.target.value)}
                    rows={4}
