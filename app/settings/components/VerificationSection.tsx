...
export function VerificationSection({ section }: VerificationSectionProps) {
  const allPass = section.checks.every((c: any) => c.status === 'pass');
  const headerColor = ({
    pass: 'text-success',
    fail: 'text-error',
    warn: 'text-warning',
    info: 'text-cyan-300',
  } as Record<string, string>)[section.status] || 'text-secondary';
  ...
  const icon = ({
    ...
    warn: '!',
    info: 'i',
  } as Record<string, string>)[section.status] || 'i';
...
function CheckRow({ check }: CheckRowProps) {
  const icon = ({
    ...
    warn: '!',
    info: 'i',
  } as Record<string, string>)[check.status] || 'i';
  ...
  const iconColor = ({
    ...
    warn: 'text-warning',
    info: 'text-cyan-300',
  } as Record<string, string>)[check.status] || 'text-tertiary';
...
interface SdkValidationNoteProps {
  coreReady?: any;
  liveProof?: any;
}

function SdkValidationNote({ coreReady, liveProof }: SdkValidationNoteProps) {
  if (liveProof) {
    return (
      <div className="px-5 py-4">
