...
function timeUntil(dateStr: string | null | undefined) {
  if (!dateStr) return '';
...
interface AgentIdentityPanelProps {
  highlightPairingId?: string | null;
}

export default function AgentIdentityPanel({ highlightPairingId }: AgentIdentityPanelProps) {
  ...
  const [approvedPairings, setApprovedPairings] = useState<any[]>([]);
  ...
  const approveOne = async (id: string, { skipRefresh = false }: { skipRefresh?: boolean } = {}) => {
    setError(null);
    try {
  ...
  const updatePermissionLevel = async (pairingId: string, permission_level: string) => {
    setError(null);
    try {
  ...
  const approvedPairingByAgent = approvedPairings.reduce((acc: Record<string, any>, p: any) => {
    acc[p.agent_id] = p;
    return acc;
  }, {});

  const revokeIdentity = async (agentId: string) => {
    setError(null);
    try {
