  ...
  if (code === 'blocked_by_policy') {
    ...
    return (
      <div role="alert" className="space-y-2 rounded-lg border border-error/20 bg-error-subtle px-3 py-3 text-sm text-error">
        ...
        {gd.reasons?.length ? <ul className="list-disc pl-5 text-xs">{gd.reasons.map((r, i) => <li key={i}>{r}</li>)}</ul> : null}
        {gd.matched_policies?.length ? (
          <div className="text-xs text-secondary">Matched: {gd.matched_policies.map((p) => p.name || p.id || p).join(', ')}</div>
        ) : null}
      </div>
    );
  }
...
export default function CapabilityInvokePanel({ fields = [], isSubmitting, result, onSubmit }) {
