...
- **SDK methods targeting archived endpoints — NEEDS OWNER-GATED RELEASE DECISION**: the dead-page finding surfaced a larger SDK gap. Node `submitFeedback` → `POST /api/feedback` and the Python routing suite (`list_routing_agents`, `register_routing_agent`, `get_routing_agent`, `update_routing_agent_status`, …) → `/api/routing/*` all call endpoints that exist ONLY under `app/api/_archive/` (no `next.config` rewrite → 404). Removing them is a **breaking change to both published SDKs** (major bump + full doc/parity/sdk:count refresh + `npm run release:sdks`, owner-published). Not done in the frontend pass — flagged for a deliberate SDK release. Alternative if a breaking major is undesirable: `@deprecated` annotations (non-breaking) noting the endpoints are archived.
