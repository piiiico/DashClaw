...
| `x402CommandPatterns` | string[] | `["agentcash[\\s@][\\s\\S]*\\bfetch\\b"]` | Case-insensitive regexes matched against a `bash`/`exec` command to identify an x402 **payment**. The default matches the agentcash `fetch` subcommand; a free `agentcash check` is intentionally not matched. |
