...
## Operator approval gates (DashClaw-specific — overrides "don't stop until done")

This run has HARD approval gates (spec §22 + the operator's dispatch instructions). The autonomous loop must NOT push through them.

Gated actions (STOP and ask): deleting major functionality; incompatible public-API change; DB schema/semantics change; money representation/precision change; currency-behavior change; adding wallet custody or payment execution; CostClaw paid unlock; historical repricing; framework/db replacement; new cross-table FK; commit / push / publish / deploy / release / open-PR; any irreversible repository operation; an unusually broad/expensive workflow swarm.

On hitting a gate:
1. Print `OPERATOR_APPROVAL_REQUIRED` with: the exact gated action, why it arose, affected files, the security/compatibility implication, and the smallest decision needed.
2. Update STATE.md status to `BLOCKED-AWAITING-APPROVAL` with the phase number.
3. Do NOT perform the gated action. Do NOT print SUPERGOAL_RUN_COMPLETE. Wait for the operator.

Safe local edits, tests, builds, docs, and incremental conversion do NOT trip a gate. The migration is behavior-preserving by design, so most phases never reach a gate — this is a backstop, not a routine pause. The end-state condition additionally requires: no `OPERATOR_APPROVAL_REQUIRED` left pending.
